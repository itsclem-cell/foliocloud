...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!('speechSynthesis' in window)) {
    ...
    function getPageText() {
      // Grab text from <main> only — skips nav, toolbar, footer
      const main = document.getElementById('main-content') || document.querySelector('main');
      if (!main) return document.title;
      // Clone so we can remove decorative/hidden bits
      const clone = main.cloneNode(true);
      clone.querySelectorAll('[aria-hidden="true"], .visually-hidden, .cookie-banner, script, style').forEach(el => el.remove());
    ...
    function startReading() {
      ...
      const chunks = text.match(/.{1,200}(\s|$)/g) || [text]; // chunk to avoid browser limits
      let i = 0;

      function speakNext() {
        if (i >= chunks.length || !speaking) { stopReading(); return; }
    ...
    // Stop if page visibility changes (e.g. user switches tab)
    document.addEventListener('visibilitychange', () => { if (document.hidden && speaking) stopReading(); });
  }
